package cn.can.tvlib.httpUtils.okhttp.model;
...
public class RequestParams {
    ...
    /** 普通的键值对参数 */
    public ConcurrentMap<String, String> urlParamsMap;

    /** 文件的键值对参数 */
    public ConcurrentMap<String, FileWrapper> fileParamsMap;

    private void init() {
        urlParamsMap = new ConcurrentHashMap<String, String>();
        fileParamsMap = new ConcurrentHashMap<String, FileWrapper>();
    }
    ...
    public String toString() {
        ...
        for (ConcurrentMap.Entry<String, String> entry : urlParamsMap.entrySet()) {
            if (result.length() > 0) result.append("&");
            result.append(entry.getKey()).append("=").append(entry.getValue());
        }
        for (ConcurrentMap.Entry<String, FileWrapper> entry : fileParamsMap.entrySet()) {
            if (result.length() > 0) result.append("&");
            result.append(entry.getKey()).append("=").append(entry.getValue());
        }
